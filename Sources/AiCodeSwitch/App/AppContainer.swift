...
struct AppContainer {
    ...
    static func liveOrCrash() -> AppContainer {
        do {
            ...
            let openClawProfileRepository = OpenClawProfileRepository(authProfilesPath: paths.openClawAuthProfilesPath)
            let openClawSwitchService = OpenClawSwitchService(profileRepository: openClawProfileRepository)
            ...
            fatalError("Failed to bootstrap AiCodeSwitch: \(error.localizedDescription)")
        }
    }
}
