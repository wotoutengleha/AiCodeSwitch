...
final class LaunchAtStartupService: LaunchAtStartupServiceProtocol, @unchecked Sendable {
    func setEnabled(_ enabled: Bool) async throws {
        #if os(macOS)
        try await performSystemRegistration(enabled: enabled)
        #else
        _ = enabled
        #endif
    }
    ...
    #if os(macOS)
    private func performSystemRegistration(enabled: Bool) async throws {
        try await Task.detached(priority: .userInitiated) { [self] in
            if enabled {
                try self.registerMainAppIfNeeded()
            } else {
                try self.unregisterMainAppIfNeeded()
            }
        }.value
    }
